...
​
￼
和当前的输入生成。这种内部的固有顺阻碍了训练样本的并行化，在序列较长时，这个问题变得更加严重，因为内存的限制限制了样本之间的批处理。最近的工作通过因子分解技巧[21]和条件计算[32]在计算效率方面取得了显著的提高，同时也提高了后者的模型性能。然而，顺序计算的基本约束仍然存在。
...
​
￼
) 。在每一步，模型都是自回归的[10]，在生成下一个字符时，将先前生成的符号作为附加输入。
...
Encoder:encoder由N(N=6)个完全相同的layer 堆叠而成.每层有两个子层。第一层是multi-head self-attention机制，第二层是一个简单的、位置全连接的前馈神经网络。我们在两个子层的每一层后采用残差连接[11]，接着进行layer normalization[1]。也就是说，每个子层的输出是
L a y e r N o r m ( x + S u b l a y e r ( x ) ) LayerNorm(x + Sublayer(x))
￼
LayerNorm(x
￼
+
...
￼
￼
外，与我们的算法相同。Additive attention使用一个具有单隐层的前馈神经网络来计算兼容性函数。尽管在理论上两者的复杂度相似，但是在实践中dot-product attention要快得多，而且空间效率更高，这是因为它可以使用高度优化的矩阵乘法代码来实现。
  当
d k d_k
￼
d
...
​
￼
比较大时，addtitive attention表现优于 dot-product attention。我们认为对于大的
d k d_k
￼
d
￼
...
​
￼
，点积在数量级上增长的幅度大，将softmax函数推向具有极小梯度的区域
4 ^4
￼
￼
4
...
​
￼
维度的queries,keys,values执行一个attention函数，我们发现使用不同的学习到的线性映射把queries, keys 和 values线性映射到
d k d_k
￼
d
￼
...
​
￼
维度h次是有益的。在queries,keys和values的每个映射版本上，我们并行的执行attention函数，生成
d v d_v
￼
d
￼
...
​
￼
维输出值。它们被拼接起来再次映射，生成一个最终值，如 Figure 2 中所示。
...
￼
o
w h e r e 　 h e a d i = A t t e n t i o n ( Q W i Q , K W i K , V W i V ) where　head_i=Attention(QW^Q_i,KW^K_i,VW^V_i)
￼
where　head
￼
i
...
  其中，映射为参数矩阵
W i Q ∈ R d m o d e l × d k W_i^Q ∈ ℝ^{d_{model}×d_k}
￼
W
￼
i
...
￼
 , 
W i K ∈ R d m o d e l × d k W_i^K ∈ ℝ^{d_{model}×d_k}
￼
W
￼
i
...
￼
 , 
W i V ∈ R d m o d e l × d v W_i^V ∈ ℝ^{d_{model}×d_v}
￼
W
￼
i
...
￼
 及
W O ∈ R h d v × d m o d e l W^O ∈ ℝ^{hd_v×d_{model}}
￼
W
￼
O
...
d k = d v = d m o d e l / h = 64 d_k = d_v = d_{model}/h = 64
￼
d
￼
k
...
￼
￼
64。 由于每个head尺寸上的减小，总的计算成本与具有全部维度的单个head attention相似。
￼
...
KaTeX parse error: Expected 'EOF', got '&' at position 41: …1 )W_2 + b_2 &̲emsp;&emsp;&ems…
...
​
￼
的向量。我们还使用普通的线性变换和softmax函数将decoder输出转换为预测的下一个词符的概率。在我们的模型中，两个嵌入层之间和pre-softmax线性变换共享相同的权重矩阵，类似于[30]。 在嵌入层中，我们将这些权重乘以
d m o d e l \sqrt{d_{model}}
￼
￼
d
￼
...
​
￼
相同，所以它们两个可以相加。有多种位置编码可以选择，例如通过学习得到的位置编码和固定的位置编码[9]
...
P E ( p o s , 2 i ) = s i n ( p o s / 1000 0 2 i / d m o d e l ) P E ( p o s , 2 i + 1 ) = c o s ( p o s / 1000 0 2 i / d m o d e l ) PE_{(pos,2i)} = sin(pos/10000^{2i/d_{model}})\\ PE_{(pos,2i+1)} = cos(pos/10000^{2i/d_{model}})
￼
PE
￼
(pos,2i)
...
￼
d
，比如在典型的序列转换的encoder或decoder中的隐藏层。我们考虑三个方面，最后促使我们使用self-attention。
...
￼
￼
n的单层卷积不会连接每一对输入和输出的位置。要这么做，在相邻的内核情况下，需要一个n个卷积层的堆栈， 在扩展卷积的情况下需要
O ( l o g k ( n ) ) O(logk(n))
￼
O(logk(n)) 层[18]，它们增加了网络中任意两个位置之间的最长路径的长度。 卷积层通常比循环层代价更昂贵，这与因子k有关。然而，可分卷积[6]大幅减少复杂度到
O ( k ⋅ n ⋅ d + n ⋅ d 2 ) O(k ⋅n⋅d + n⋅d^2)
￼
O(k
￼
⋅
...
￼
￼
n，可分离卷积的复杂度等于self-attention layer和point-wise feed-forward layer的组合，这是我们在模型中采用的方法。
...
w a r m u p _ s t e p s = 4000 warmup\_steps = 4000
￼
warmup_steps
￼
=
...
￼
￼
0.1[36]的标签平滑。这会影响ppl,因为模型学习到了更多的不确定性，但提高了准确率和BLEU评分。
￼
￼
6 Results（结果）
￼
...
​
￼
 = 1024 在Penn Treebank[25]的Wall Street Journal（WSJ）部分训练了一个4层的transformer，约40K个训练句子。 我们还使用更大的高置信度和BerkleyParser语料库，在半监督环境中对其进行了训练，大约17M个句子[37]。 我们使用了一个16K词符的词汇表作为WSJ唯一设置，和一个32K词符的词汇表用于半监督设置。
